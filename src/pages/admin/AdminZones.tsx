...
export default function AdminZones() {
  return <SidebarLayout><ZonesView /></SidebarLayout>;
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-foreground">Zone Management</h2>
      ...
      {isLoading ? (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map(i => <div key={i} className="glass-card h-24 animate-pulse bg-secondary/50" />)}
        </div>
      ) : !zones?.length ? (
...
function PremiumFeeEditor({ zone }: { zone: any }) {
  ...
  if (!editing) {
    return (
      <button onClick={() => setEditing(true)} className="mt-2 flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground transition-colors">
  ...
  return (
    <div className="mt-2 flex items-center gap-2">
      ...
      <Input type="number" value={fee} onChange={(e) => setFee(e.target.value)} className="h-8 w-24 rounded-lg bg-secondary border-border text-sm" min={0} />
      <Button size="sm" className="h-8 rounded-lg text-xs" onClick={() => { updateZone.mutate({ id: zone.id, premium_fee: parseFloat(fee) || 0 }); setEditing(false); }}>Save</Button>
      <Button size="sm" variant="ghost" className="h-8 rounded-lg text-xs" onClick={() => setEditing(false)}>Cancel</Button>
    </div>
  );
}
...
function CreateZoneForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    createZone.mutate({ name: name.trim(), slug, description: description.trim() || undefined, premium_fee: parseFloat(premiumFee) || 0 }, { onSuccess });
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <Button className="w-full rounded-xl" onClick={handleSubmit} disabled={!name.trim() || createZone.isPending}>Create Zone</Button>
    </div>
  );
}
...
function OperatorZoneAssignment() {
  ...
  return (
    <div className="mt-8">
      ...
      <div className="glass-card p-4 mb-4 max-w-lg">
        <div className="space-y-3">
          <Select value={selectedOperator} onValueChange={setSelectedOperator}>
            <SelectTrigger className="h-11 rounded-xl bg-secondary border-border"><SelectValue placeholder="Select operator" /></SelectTrigger>
            <SelectContent>
              {operators?.map(o => <SelectItem key={o.user_id} value={o.user_id}>{o.full_name || "Unnamed"}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={selectedZone} onValueChange={setSelectedZone}>
            <SelectTrigger className="h-11 rounded-xl bg-secondary border-border"><SelectValue placeholder="Select zone" /></SelectTrigger>
            <SelectContent>
              {zones?.filter(z => z.is_active).map(z => <SelectItem key={z.id} value={z.id}>{z.name}</SelectItem>)}
            </SelectContent>
          </Select>
          <Button className="w-full h-11 rounded-xl" onClick={() => assignOperatorZone.mutate({ operator_id: selectedOperator, zone_id: selectedZone })} disabled={!selectedOperator || !selectedZone || assignOperatorZone.isPending}>
            Assign Zone
          </Button>
        </div>
      </div>
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
        {operatorZones?.map((oz, i) => (
          <motion.div key={oz.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="glass-card p-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-destructive rounded-xl" onClick={() => removeOperatorZone.mutate(oz.id)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          </motion.div>
