...
export default function DispatcherDashboard() {
  return <SidebarLayout fullScreen><MonitorView /></SidebarLayout>;
}

export function DispatcherAssign() {
  return <SidebarLayout><AssignView /></SidebarLayout>;
}

export function DispatcherStats() {
  return <SidebarLayout><StatsView /></SidebarLayout>;
}
...
function MonitorView() {
  ...
  return (
    <div className="relative flex h-[calc(100dvh-56px)] lg:h-screen flex-col">
      ...
      <div className="relative z-20 mt-auto">
        <div className="map-gradient-bottom pt-8 pb-6 lg:pb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {riders.map(rider => (
                    <Button key={rider.user_id} size="sm" variant="outline" className="rounded-xl text-xs"
                      onClick={() => assignMutation.mutate({ rideId: ride.id, riderId: rider.user_id })} disabled={assignMutation.isPending}>
                      <Users className="mr-1 h-3 w-3" />{rider.full_name || "Rider"}
                    </Button>
                  ))}
                </div>
              ) : (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.06 }} className="glass-card p-5">
